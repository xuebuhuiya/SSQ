...
- Claude Code 调用 DeepSeek API 负责读取项目、提出计划、实现代码、运行测试。
- Codex 负责审查计划、检查 diff、复核测试日志、提出最小修复意见。
...
### Step 1: DeepSeek 读取项目并提出计划

让 Codex 调用 Claude Code，只读项目并输出计划：

```text
调用 Claude Code/DeepSeek 读取当前项目，只提出实现计划、风险点和需要修改的文件，不要改文件。
任务：...
```

### Step 2: Codex 审查计划

Codex 检查计划是否过宽、是否会影响安全/鉴权/数据库/构建流程。

简单任务可直接进入执行。高风险任务需要先收窄范围。

### Step 3: DeepSeek 修改代码并运行测试

推荐 prompt：

```text
调用 Claude Code/DeepSeek 按刚才计划实现。
要求：
- 只修改必要文件
- 不要 commit
- 不要 push
...
先让 DeepSeek 只读项目并提出计划，不要改文件。你审查计划后再决定是否让它执行。
```
...
可以直接让 DeepSeek 实现，但不要 commit/push。完成后你审查 diff 并跑测试。
```
