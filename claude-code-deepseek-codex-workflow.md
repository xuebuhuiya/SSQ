...
- 大任务必须拆成多个小批次，每批只包含少量文件和一个清晰验收点；
...
- 每批最好控制在 1–5 个文件；
- 每批只解决一个层次的问题，例如“项目骨架和配置”“数据读取和清洗”“核心业务逻辑”“测试补齐”；
- 每批都要能独立运行局部测试或全量测试；
- 不要一次性让 DeepSeek 创建完整项目、实现全部模块、写完全部测试并运行测试。
...
### Step 4: DeepSeek 按小批次修改代码并运行测试

推荐 prompt：

```text
调用 Claude Code/DeepSeek 按以下最终计划中的“当前批次”实现。
...
- DeepSeek 每完成一个批次，Codex 必须先审查 `git diff` 和测试结果；
- 当前批次未通过测试前，不进入下一批；
- 如果 Claude Code 超时、卡在权限确认、或出现晚到写入，Codex 立即接手检查 `git status` / `git diff`，必要时恢复到上一个干净状态；
- 只有当一个批次干净通过后，才继续下一个批次。
...
- 不要覆盖用户已有改动
- 完成后必须说明修改文件和测试结果
- 单次任务只处理当前批次，不要顺手做后续批次
- 如果需要写入批准或权限确认，先停下来说明，不要等待交互
```
...
Codex 审查复核意见后，把最终计划拆成多个小批次。
每次只让 DeepSeek 执行一个批次；每批完成后 Codex 都要审查 diff 并跑测试，再决定是否进入下一批。
```
...
要求不要 commit/push。即使是小任务，也只执行当前明确范围，不要顺手扩展。
完成后 Codex 审查 diff 并跑测试。
```
